...
function M = cubesat_Alicino_nominal()



% nominal values for AOCS 
%---design-----------------------------------------------------------------
x_aocs(1) = 30;         %%%       * x(1) = pointing accuracy, deg                                                (fix, 30)   
x_aocs(2) = 0.01;       %       * x(2) = offset between centre of gravity and centre of pressure, m   (ALICINO uncertain, [0.005 0.01][0.01 0.02])
x_aocs(3) = 0.0885;     %       * x(3) = area normal to velocity vector, m^2                          (ALICINO uncertain, [0.034 0.0885][0.0885 0.15])
x_aocs(4) = 0.0885;     %       * x(4) = area normal to sun line, m^2                                 (ALICINO uncertain, [0.034 0.0885][0.0885 0.15])
x_aocs(5) = 0.6;        %       * x(5) = reflectance factor                                           (ALICINO uncertain, [0.5 0.6][0.6 0.7])
x_aocs(6) = 0.001;      %       * x(6) = spacecraft residual dipole, A*m^2                            (ALICINO uncertain, [0.0005 0.001][0.001 0.0015])
x_aocs(7) = 2.2;        %       * x(7) = drag coefficient                                             (ALICINO uncertain, [2 2.2][2.2 2.5])
...
x_aocs(11) =  10;       %       * x(11) = slew angle, deg                                             (ALICINO design, [10 60])
x_aocs(12) =  90;       %       * x(12) = time for slew maneuvre, s                                   (ALICINO design, [30 90])
...
%---design-----------------------------------------------------------------
x_ttc(1) = 8.253;       %       * x(1) = frequency, GHz                           (ALICINO design       [7 10])
x_ttc(2) = 1;           %       * x(2) = modulation,                              (ALICINO design       [0 1])
x_ttc(3) = 0.8;         %       * x(3) = antenna efficiency                       (ALICINO uncertainty  [0.6 0.8] [0.8 0.9])
x_ttc(4) = 3;           %       * x(4) = antenna gain, dB                         (ALICINO uncertainty  [1 3][3 5] trasmit AG)
x_ttc(5) = 0.5;         %       * x(5) = onboard loss, dB                         (ALICINO uncertainty  [0.1 0.5][0.5 1])
x_ttc(6) = 2;           %       * x(6) = other unmodelled losses, dB              (ALICINO uncertainty  [0.5 1.5][1.5 2])
x_ttc(7) = 0.3;         %       * x(7) = mass of distribution network, kg         (ALICINO uncertainty  [0.1 0.3][0.2 0.5])
x_ttc(8) = 0.5;         %%%       * x(8) = modulation index, rad
x_ttc(9) = 0;           %       * x(9) = amplifier type,                          (ALICINO design       [0 1])
%---epistemic--------------------------------------------------------------
...
%--------------------------------------------------------------------------
x_power(1) =  0;         %       * x(1) = type of solar cell                                                                                       (design, [0 1])
x_power(2) =  5;         %       * x(2) = required bus voltage, V [0,100]                                                                  (ALICINO design, [3 5])                                                                                             
x_power(3) =  1;         %       * x(3) = eps configuration (det, mppt) [0,0.5) = det; [0.5,1] = mppt                                      (ALICINO design, [0 1])
x_power(4) =  0.85;      %       * x(4) = cell packing efficiency (or assembly factor) [0,1]                                               (ALICINO uncertain, [0.8 0.85][0.85 0.9])   
...
x_power(8) =   1;        %       * x(8) = allowable voltage drop,                                                                          (ALICINO design, [1 3])              
...
ep_power(7) = 10;        %       * ep(7) = uncertainty on array temperature, C                   (ALICINO uncertain, [0 10][10 15])
...
ep_power(10) = 10;       %       * ep(10) = uncertainty on power requirements, %                 (ALICINO uncertain, [0 10][10 20])
...
ep_power(13) = 0;       %        uncertain         Delta rho_sa [% kg/m^2] density solar array, [0 15][15 30]
ep_power(14) = 0;       %        uncertain         Delta D_cell [%] degradation solar array,    [0 50][50 100]
%--------------------------------------------------------------------------
