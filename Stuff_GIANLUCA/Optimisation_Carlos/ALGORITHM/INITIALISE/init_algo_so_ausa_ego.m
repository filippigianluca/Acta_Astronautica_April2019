function [algo_minmax, algo_outer, algo_inner] = init_algo_so_ausa_ego(minmax_problem);

%% META ALGORITHM: MACSMINMAX
algo_minmax.optimise = @optimise_ausa_ego;            % algorithm in the form [x,f,exitflag,output] = algo(problem,algo_outer,algo_inner,par_minmax)
% parameters
    if isfield(minmax_problem,'maxnfeval_eff')
        par_minmax.maxnfeval = minmax_problem.maxnfeval_eff;% TOTAL function evaluation limit
    else
    ...
    % initial sample of U for each EGO inner-loop optimisation
    par_minmax.n_u0 = min(max(10,3*minmax_problem.dim_u), 30);

    par_minmax.indicator_u_max = 1e-6; % Threshold on the indicator (EI, PI, etc.) for inner loop
...
if (minmax_problem.n_obj == 1)
    ...
    algo_outer.optimise_au = @optimise_mpaidea_wrapper;       % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = -1*minmax_problem.dim_d;     % number of function evaluations for IDEA
    ...
    algo_outer.optimise_sa = @optimise_mpaidea_wrapper;       % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = min(1e4,2e3*minmax_problem.dim_d);     % number of function evaluations for IDEA
        par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
        par_mpaidea.n_agents = max(4,minmax_problem.dim_d);   % number of agents in one population
        par_mpaidea.population=[];                            % initial population, same for all execution. Leave empty to randomize.
        par_mpaidea.max_LR = 5;                              % max.number of local restarts
...
%% ALGORITHM OUTER LOOP with archive cross-checks
    ...
    % %parameters{
    %     par_macs = struct;
    %     par_macs.maxnfeval = 0*minmax_problem.dim_d;      % Max function evaluations
    ...
    % %parameters{
    %     par_macs = struct;
    %     par_macs.maxnfeval = 2e3*minmax_problem.dim_d;      % Max function evaluations
    %     par_macs.popsize = 8 + minmax_problem.dim_d;        % Population size
    %     par_macs.rhoini = 1;                                % Initial local hypercube size
    %     par_macs.F = 0.9;                                   % F
    %     par_macs.CR = 0.9;                                  % CR
...
for obj = 1:minmax_problem.n_obj
    ...
    par_mpaidea = struct;
        par_mpaidea.nFeValMax = min(2.5e3,500*minmax_problem.dim_u);     % number of function evaluations for IDEA
        par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
        par_mpaidea.n_agents = max(4,2*minmax_problem.dim_u); % number of agents in one population
        par_mpaidea.population=[];                            % initial population, same for all execution. Leave empty to randomize.
        par_mpaidea.max_LR = 5;                               % max.number of local restarts
        par_mpaidea.DE_strategy = 1;                          % 1: DE/Rand and DE/CurrentToBest; 2: DE/Rand and DE/Best
        par_mpaidea.prob_DE_strategy = 0.5;                   % probability or not using DE/Rand
