function [algo_minmax, algo_outer, algo_inner] = init_algo_relax_bisurr_advanced_checks(minmax_problem);

%% META ALGORITHM: MACSMINMAX
algo_minmax.optimise = @optimise_relax_bisurr_advanced_checks;            % algorithm in the form [x,f,exitflag,output] = algo(problem,algo_outer,algo_inner,par_minmax)
% parameters
    ...
    cc_opt = struct;
        ...
        cc_opt.max_dom_idx_ref = 5;           % guys with dom_idx <= this are considered reference. Recommended >> 0 for SO and lower for MO.
...
if (minmax_problem.n_obj == 1)
    warning('using MO settings for a SO problem!')
