function [algo_minmax, algo_outer, algo_inner] = init_algo_mo_ausane(minmax_problem);

%% META ALGORITHM: MACSMINMAX
algo_minmax.optimise = @optimise_mo_ausane;            % algorithm in the form [x,f,exitflag,output] = algo(problem,algo_outer,algo_inner,par_minmax)
% parameters
    if isfield(minmax_problem,'maxnfeval')
        par_minmax.maxnfeval = minmax_problem.maxnfeval*500;% TOTAL function evaluation limit
    else
...
if (minmax_problem.n_obj == 1)
    ...
    %% ALGORITHM OUTER LOOP for nested optimisation
    algo_outer.optimise_ne = @optimise_macs_wrapper;           % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
    %parameters{
        par_macs.maxnfeval = 60*minmax_problem.dim_d;      % Max function evaluations
        par_macs.popsize = 3;                               % Population size
        par_macs.rhoini = 1;                                % Initial local hypercube size
        par_macs.F = 0.9;                                   % F
        par_macs.CR = 0.9;                                  % CR
        par_macs.p_social = 0.2;                            % Ratio between elite and total population
    ...
    algo_outer.par_ne = par_macs;

end

algo_inner = cell(1,minmax_problem.n_obj);
for obj = 1:minmax_problem.n_obj
    %% ALGORITHM INNER LOOP: MPAIDEA, used for all maximisations including nested ones.
