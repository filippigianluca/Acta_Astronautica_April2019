function [algo_minmax, algo_outer, algo_inner] = init_algo_so_ausa_tc16(minmax_problem);
...
% parameters
    ...
    par_minmax.maxnfeval = 1.5e4  ;% TOTAL function evaluation limit
    ...
    par_minmax.max_iter_alternation = 6;
...
if (minmax_problem.n_obj == 1)
    ...
    algo_outer.optimise_au = @optimise_mpaidea_wrapper;       % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = 2e2*minmax_problem.dim_d;     % number of function evaluations for IDEA
    ...
    algo_outer.optimise_sa = @optimise_mpaidea_wrapper;       % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = 2e3*minmax_problem.dim_d;     % number of function evaluations for IDEA
        par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
        par_mpaidea.n_agents = max(4,minmax_problem.dim_d);   % number of agents in one population
        par_mpaidea.population=[];                            % initial population, same for all execution. Leave empty to randomize.
        par_mpaidea.max_LR = 5;                              % max.number of local restarts
...
%     %parameters{
%         par_macs = struct;
%         par_macs.maxnfeval = 1e2*minmax_problem.dim_d;      % Max function evaluations
...
obj = 1;
    ...
    par_mpaidea = struct;
        par_mpaidea.nFeValMax = 1e2*minmax_problem.dim_u;     % number of function evaluations for IDEA
        par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
        par_mpaidea.n_agents = 3;                             % number of agents in one population
        par_mpaidea.population=[];                            % initial population, same for all execution. Leave empty to randomize.
