function [algo_minmax, algo_outer, algo_inner] = init_subalgo_sdu_ausa(minmax_problem);

%% META ALGORITHM: MACSMINMAX
algo_minmax.optimise = @optimise_ausa;            % algorithm in the form [x,f,exitflag,output] = algo(problem,algo_outer,algo_inner,par_minmax)
    
    % parameters
    par_minmax.maxnfeval = inf;
    par_minmax.max_iter = 10;

    par_minmax.verbosity = false;
...
if (minmax_problem.n_obj == 1)
    ...
    algo_outer.optimise_au = @optimise_mpaidea_wrapper;       % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = 500*minmax_problem.dim_d;     % number of function evaluations for IDEA
...
for obj = 1:minmax_problem.n_obj
    ...
    algo_inner{obj}.par.psi_surr = false;
end
