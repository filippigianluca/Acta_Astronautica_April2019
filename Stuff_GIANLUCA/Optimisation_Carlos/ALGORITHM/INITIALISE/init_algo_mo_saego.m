function [algo_minmax, algo_outer, algo_inner] = init_algo_mo_saego(minmax_problem);

%% META ALGORITHM: MACSMINMAX
algo_minmax.optimise = @optimise_mo_saego;            % algorithm in the form [x,f,exitflag,output] = algo(problem,algo_outer,algo_inner,par_minmax)
% parameters
    if isfield(minmax_problem,'maxnfeval')
        par_minmax.maxnfeval = minmax_problem.maxnfeval/5;% TOTAL function evaluation limit
    else
    ...
    par_minmax.n_d0 = 8;
    par_minmax.n_u0 = 9;
    par_minmax.n_u_record_0 = repmat([1],[1,minmax_problem.n_obj]); %HC
