function [d,fval,exitflag,output] = optimise_relax(problem_minmax, algo_outer, algo_inner, par_minmax)
...
% for i = 1:size(d_0,1)
    ...
    for obj = 1:n_obj
        u_record{obj} = rand(1,n_u);   
        % problem_max_u.par_objfun.objective = obj;                                     % tell metaproblem what objective to optimise
        % [ umax, ~ , ~ , output_aux ] = algo_inner{obj}.optimise(problem_max_u,algo_inner{obj}.par); % optimise
...
while ~stop
    ...
    [dmin, f_outer, ~ , output_aux] = algo_outer.optimise_au(problem_min_d,algo_outer.par_au);

    nfeval = nfeval + (output_aux.nfeval+(n_obj>1))*size_u_record; 
    ...
    % Remove solutions output more than once (if any)
    ...
    stop = nfeval >= nfevalmax;
end

sel = dominance(f_record_aux,0) == 0;
fval = f_record_aux(sel,:);
frontsize = size(fval,1);
dval = dmin(sel,:);
d = dval.*repmat(problem_minmax.ub_d'-problem_minmax.lb_d',[frontsize,1]) + repmat(problem_minmax.lb_d',[frontsize,1]);
[~, u_val_record, ~] = u_validation(problem_max_u, dval, u_record, false, 1:n_obj);
