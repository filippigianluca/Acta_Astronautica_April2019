function [d,fval,exitflag,output] = optimise_mo_ausane(problem_minmax, algo_outer, algo_inner, par_minmax)
...
problem_inner_ne = build_metaproblem_nested_inner(problem_minmax);
problem_outer_ne = build_metaproblem_nested_outer(problem_inner_ne, algo_inner);
...
%compute fevals inner nested
fevals_inner_ne = 0;
for obj = 1:n_obj
    fevals_inner_ne = fevals_inner_ne + algo_inner{obj}.par.nFeValMax; %this is IDEA-dependent...
end
...
%             end
        ...
        for obj = 1:n_obj
            ...
            if sign_inner*f_inner > sign_inner*f_outer_au(i,obj)
                if lsflag_inner
                    u_cell_aux = cell(1,n_obj);
                    u_cell_aux{obj} = umax;
                    [fmax2,umax2,nfeval2] = u_validation(problem_max_u, dmin_au(i,:), u_cell_aux, lsflag_inner, obj);
                    nfeval = nfeval + nfeval2;
                ...
                if lsflag_inner
                    % Evaluate subproblem to find umax2 = arg max f(dmin(i),ua)
                    [fmax2,umax2,nfeval2] = u_validation(problem_max_u, dmin_au(i,:), u_record, lsflag_inner, obj);
                    nfeval = nfeval + nfeval2;
                    u_record{obj} = [u_record{obj}; umax2{obj}];
                    f_record_aux(size(dmin_sa,1)+i,obj) = fmax2(1,obj);
                else
    ...
    % dunno how effective or necessary this is
    ...
    % refine some points with nested optimisation
    oldarch = d_record;
    for obj = 1:n_obj
        oldarch = [oldarch, u_val_record_aux{obj}];
    end
    oldarch = [oldarch, f_record, zeros(size(f_record, 1),2)];
    oldarch(:,end) = dominance(f_record,0);

    [memory_pop_nested,dd,energy,ener2,mins,maxs]=arch_shrk6([],[],oldarch(1:algo_outer.par_ne.popsize,:),0,[],min(f_record(1:algo_outer.par_ne.popsize,:)),max(f_record(1:algo_outer.par_ne.popsize,:)),size(d_record,2),size(f_record,2),algo_outer.par_ne.popsize);

    [memory_pop_nested,dd,energy,ener2,mins,maxs]=arch_shrk6(memory_pop_nested,dd,oldarch(algo_outer.par_ne.popsize+1:end,:),energy,ener2,min(f_record(algo_outer.par_ne.popsize+1:end,:)),max(f_record(algo_outer.par_ne.popsize+1:end,:)),size(d_record,2),size(f_record,2),algo_outer.par_ne.popsize);
    
    algo_outer.par_ne.initial_memory = memory_pop_nested;

    [du_ne, f_ne, ~ , output_aux] = algo_outer.optimise_ne(problem_outer_ne,algo_outer.par_ne);
    
    nfeval = nfeval + output_aux.nfeval*fevals_inner_ne;


    % add to the archive the u's from nested opt.
    for obj = 1:n_obj
        u_record{obj} = [u_record{obj}; du_ne(:,n_d+(obj-1)*n_u+1:n_d+obj*n_u)];
        [~, idx] = unique(round(1e8*u_record{obj}),'rows');
        u_record{obj} = u_record{obj}(idx,:);
    end

    % validate the ds from nested opt
    d_ne = du_ne(:,1:n_d);
    [f_ne, u_val_record_ne, nfeval_aux] = u_validation(problem_max_u, d_ne, u_record, false, 1:n_obj);
    nfeval = nfeval + nfeval_aux;

    % add the d's and f's from nested opt
    d_record = [d_ne; d_record];
    f_record = [f_ne; f_record];
    % Remove solutions archived more than once (if any)
    [~, idx] = unique(round(1e8*d_record),'rows');
    d_record = d_record(idx,:);
    f_record = f_record(idx,:); %not worrying too much by loss of validated solutions
    ...
    if (n_obj == 2)
        ...
        colors = 'ckbrmg';
        hold on
        plot(f_ne(:,1),f_ne(:,2),strcat(colors(mod(iter,length(colors))+1),'*'))
        drawnow
        % figure(1)    
    end
    f_ne
    ...
    %     colors = 'ckbrmg';
    %     hold on
    %     plot(f_record(:,1),f_record(:,2),strcat(colors(mod(iter,length(colors))+1),'^'))
    %     drawnow
    %     % figure(1)    
    % end
