function [d,fval,exitflag,output] = optimise_ausa(problem_minmax, algo_outer, algo_inner, par_minmax)
...
while ~stop
    ...
    if (any(psi_surr) || algo_outer.par_sa.use)
        % manage dataset
    ...
    dmin = [dmin_au; dmin_sa];
    f_outer = [f_outer_au; f_outer_sa];
    % Remove solutions returned more than once (if any)
