function [d,fval,exitflag,output] = optimise_ausa_minmarek(problem_minmax, algo_outer, algo_inner, par_minmax)
...
if algo_outer.par_sa.use ||  any(psi_surr)
    problem_min_d_sa = build_metaproblem_minmarek_outer(problem_minmax);
end
...
% Initialise archive with EGO maximisations
...
for i = 1:size(d_0,1)
    ...
    for obj = 1:n_obj
        ...
        u_ego_inner = lhsdesign(n_u0,n_u,'criterion','maximin');

        algo_inner{obj}.par.initial_population= [];
        f_ego_inner =[];
        for j = 1:size(u_ego_inner,1);
            f_ego_inner(j,1)=problem_max_u.objfun(u_ego_inner(j,:),problem_max_u.par_objfun); % note it comes negative for minmax
            %nfeval = nfeval + 1;
        end
        ...
        while abs(indicator_u) >= indicator_u_max && iter_u < iter_u_max
            ...
            [~,idx] = unique(round(1e8*u_ego_inner),'rows');
            u_ego_inner = u_ego_inner(idx,:);
            f_ego_inner = f_ego_inner(idx,:);
            problem_inner.par_objfun.surrogate.model = problem_inner.par_objfun.surrogate.training(u_ego_inner,f_ego_inner,problem_inner.par_objfun.surrogate);
            problem_inner.par_objfun.ymin = min(f_ego_inner);

            problem_inner.par_objfun.use_indicator = true;
            % maximise indicator_u
            [u_ego_inner_aux, indicator_u, ~, ~] = algo_inner{obj}.optimise(problem_inner,algo_inner{obj}.par); %no nfeval here
            indicator_u = -indicator_u;

            u_ego_inner_aux(u_ego_inner_aux < 0) = 0;
            u_ego_inner_aux(u_ego_inner_aux > 1) = 1;

            f_ego_inner_aux = problem_max_u.objfun (u_ego_inner_aux, problem_max_u.par_objfun); %evaluate. note it returns negative for minmax
            %nfeval = nfeval + 1;
            
            %apend u and f
            u_ego_inner = [u_ego_inner; u_ego_inner_aux];
            f_ego_inner = [f_ego_inner; f_ego_inner_aux];
        ...
        [u_ego_inner_aux2, ~, ~, ~] = algo_inner{obj}.optimise(problem_inner,algo_inner{obj}.par);
        u_ego_inner_aux2(u_ego_inner_aux2 < 0) = 0;
        u_ego_inner_aux2(u_ego_inner_aux2 > 1) = 1;
        f_ego_inner_aux2 = problem_max_u.objfun (u_ego_inner_aux2, problem_max_u.par_objfun);
        %nfeval = nfeval +1;

        u_ego_inner = [u_ego_inner; u_ego_inner_aux2];
        f_ego_inner = [f_ego_inner; f_ego_inner_aux2];

        [fmax, sel] = min(f_ego_inner);
        fmax = -sign_inner*fmax;
        umax = u_ego_inner(sel,:);
            
        u0i = [u0i, umax];
        fi = [fi, fmax];
        d_info(i).fmax(obj) = fmax;
