function [d,fval,exitflag,output] = optimise_ausa_trisurr_ei(problem_minmax, algo_outer, algo_inner, par_minmax)
...
if algo_outer.par_sa.use ||  any(psi_surr)
    problem_min_d_sa = build_metaproblem_ei_outer(problem_minmax);
end
...
while ~stop
    ...
    if(algo_outer.par_sa.use&&(~alternate_outer || meth_outer(i_meth_outer) == 1))
        problem_min_d_sa.par_objfun.ymin = ref(:,end-n_obj+1:end);
