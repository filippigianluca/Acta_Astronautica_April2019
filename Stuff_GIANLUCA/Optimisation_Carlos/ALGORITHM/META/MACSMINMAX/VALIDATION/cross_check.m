function [record,d_info,u_record,nfeval_cc] = cross_check(problem_fix_d,record,d_info,u_record,cc_opt)
	...
	if(isfield(cc_opt,'restrain_u_record') && cc_opt.restrain_u_record)
		for obj = 1:n_obj
			[idx_record,~] = ismember(round(cc_opt.tol*u_record{obj}), round(cc_opt.tol*record(:,n_d+(obj-1)*n_u+1 :n_d+ obj*n_u)),'rows');

			idx_record_sum = cumsum(idx_record);
			u_record{obj} = u_record{obj}(idx_record,:);

			for i=1:size(record,1)
                % try
				d_info(i).u_checked{obj} = unique(idx_record_sum(d_info(i).u_checked{obj}));
				d_info(i).u_checked{obj} = d_info(i).u_checked{obj}(d_info(i).u_checked{obj}>0);
				d_info(i).u_ls{obj} = unique(idx_record_sum(d_info(i).u_ls{obj}));
				d_info(i).u_ls{obj} = d_info(i).u_ls{obj}(d_info(i).u_ls{obj}>0);
				d_info(i).umax(obj) = idx_record_sum(d_info(i).umax(obj));
				if d_info(i).umax(obj) == 0
					warning('the umax=0 error is still there!! fix it once and for all you lazy ass.')
					keyboard
				end
                % catch
                %     d_info(i).u_checked{obj}
                %     error('hey')
                % end
			end
		end
