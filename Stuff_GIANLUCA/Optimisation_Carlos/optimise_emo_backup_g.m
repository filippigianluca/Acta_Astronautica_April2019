function [d,fval,exitflag,output] = optimise_emo_backup_g(problem_minmax, algo_outer, algo_inner, par_minmax)
...
surrogate = par_minmax.surrogate;
...
problem_inner.par_objfun.surrogate...
...
while ~stop
    ...
    while abs(indicator_d) > indicator_d_max && iter_d < iter_d_max
        ...
        problem_outer.par_objfun.surrogate...
    ...
    for i = 1:n_dmin
        ...
        for obj = 1:n_obj % list_obj
            ...
            while abs(indicator_u) > indicator_u_max && iter_u < iter_u_max
                ...
                for obj2 = 1:n_obj
                    surrogate{obj2} = surrogate{obj2}.update(surrogate{obj2});
                end
                problem_inner.par_objfun.surrogate...
                ...
                for obj2 = 1:n_obj
                    ...
                    surrogate{obj2} = surrogate{obj2}.add([dmin_outer(i,:) , u_inner_aux], f_doe_aux(1,obj2), surrogate{obj2});
                end
                nfeval = nfeval + 1;
