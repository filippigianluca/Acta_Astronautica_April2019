function [algo_minmax, algo_outer, algo_inner] = init_algo_so_sstl_corners_decomp(minmax_problem);
...
if (minmax_problem.n_obj == 1)
    ...
    algo_outer.optimise = @optimise_mpaidea_wrapper;          % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
        par_mpaidea.nFeValMax = 300*minmax_problem.dim_d;     % number of function evaluations for IDEA
...
algo_inner.optimise = @optimise_sstl_decomp;          % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
algo_inner.minimise = @minimise_sstl_decomp;
    par_mpaidea.nFeValMax = 100*minmax_problem.dim_u;     % number of function evaluations for IDEA
    par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
    par_mpaidea.n_agents = 20;%max(5,minmax_problem.dim_u);   % number of agents in one population
    par_mpaidea.population=[];                            % initial population, same for all execution. Leave empty to randomize.
