function [problem_minmax] = init_problem_transfer()
    ...
    lb_u = cell(dim_u,1);
    ub_u = cell(dim_u,1);
    lb_u{1,1} = -1;     ub_u{1,1} = +7;
    lb_u{2,1} = -1;     ub_u{2,1} = +5;
    for i=3:8
        lb_u{i,1} = -k_sigma*sigma0(i-2);
        ub_u{i,1} = +k_sigma*sigma0(i-2);
    end
    for i=9:14
        lb_u{i,1} = -k_sigma*sigma1(i-8);
        ub_u{i,1} = +k_sigma*sigma1(i-8);
    end
