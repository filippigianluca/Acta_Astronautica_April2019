function [algo_minmax, algo_outer, algo_inner] = init_algo_transfer(minmax_problem);
...
% parameters
    ...
    % Cross-check options
    cc_opt = struct;
        cc_opt.objectives_to_skip = [1];      % tweak to avoid cross-checking objective 1
        cc_opt.to_crosscheck = 2;             % 0 cross-checks no one, 1 cross-checks the reference, 2 cross-checks everyone in the archive
...
%% ALGORITHM OUTER LOOP with archive cross-checks
    ...
    %parameters{
        par_macs = struct;
        par_macs.maxnfeval = 2e2*minmax_problem.dim_d;      % Max function evaluations
...
algo_inner = cell(1,minmax_problem.n_obj);
    ...
    %% ALGORITHM INNER LOOP for tof: maximisation is direct
    algo_inner{1}.optimise = @maximise_tof_tweak;
    algo_inner{1}.par = struct;
    algo_inner{1}.par.psi_surr = false;

    %% ALGORITHM INNER LOOP for dv: MPAIDEA
    algo_inner{2}.optimise = @optimise_mpaidea_wrapper;                % algorithm in the form [x,f,exitflag,output] = algo(problem,par_algo)
    par_mpaidea = struct;
        par_mpaidea.nFeValMax = 3e2*minmax_problem.dim_u;     % number of function evaluations for IDEA
        par_mpaidea.n_populations = 1;                        % number of populations, if no adaptive behaviour should set to 1
        ...
        par_mpaidea.text = 0;    
        par_mpaidea.record = [1];                             % Fraction(s) of nfevalmax at which the result is saved
    algo_inner{2}.par = par_mpaidea;
    algo_inner{2}.par.psi_surr = true;
