...
Tmargin  = ep(7);
life  = ep(8);
bus_type = logical(ep(9)); % 0 = unregulated; 1 = regulated
Pmargin = 1 + ep(10)/100;
MeV = ep(11);
Preq = Preq*Pmargin;
Pecl = Pecl*Pmargin;
cubesat = logical(ep(12)); % 0 = big; 1 = cubesat

% det
eta.system.sar = V_bus/(V_bus + 1.2);
% mppt
eta.system.sar(configuration) = interp1([20 30 40 50 70 100],[0.93 0.941 0.952 0.96 0.966 0.99],V_bus,'linear','extrap'); % [0.93 0.941 0.952 0.96 0.966 0.97]
% unregulated
eta.system.bcr = 1; 
% regulated
...
cell_efficiency = ep(13);                           % MODIFIED  


% Battery
[M_batteries, battery] = batteries(Pecl, tecl, DOD, V_bus, eta);
M_batteries = battery.Ecap/ep(14);                  % MODIFIED
...
[M_array, P_array, array, eta] = solar_array(Preq, Pecl, tsun, tecl, range, theta, life, V_sa, cell_type, configuration, eta, MeV, Tmargin, degradation, rho_sa, cell_efficiency);
...
function [M_array, P_array, array, eta] = solar_array(Preq, Pecl, tsun, tecl, range, theta, life, V_bus, cell_type, configuration, eta, MeV, Tmargin, degradation, rho_sa, cell_efficiency)
...
cell.degradation = degradation;% 1/year    % MODIFIED 
cell.efficiency  = cell_efficiency;         % MODIFIED 
% eta.array.total  = cell_efficiency;         % MODIFIED
cell.spec_mass   = rho_sa;
...
eta.array.total = eta.array.packing*eta.array.degr*eta.array.pointing*eta.array.range*eta.array.otherfactors; %*cell_efficiency; 
...
Tmax = nthroot( abs((alpha_SE*A_est*S)/((cell.emittance + epsilon_B)*A_est*sigma)),4) - 273.15 + Tmargin;   % MODIFIED
...
global Ncell
Ncell = [Ncell array.cells_total];

global tmaxxx
tmaxxx = [tmaxxx Tmax];

%% MODIFIED: rho_sa is a epistemic parameter
% array.mass = array.area*rho_sa;            
% M_array = cell.area*array.cells_total*rho_sa;
%%
% M_array = A_est*rho_sa;
