...
%   r_TOD       Satellite position vector in TOD coordinate system [m]
...
% 
%--------------------------------------------------------------------------
function dens = nrlmsise00(Mjd_UTC, r_TOD, UT1_UTC, TT_UTC)
...
lon = atan2( r_TOD(2), r_TOD(1) );
lon = mod(lon, 2*pi);
lat = atan2( r_TOD(3), sqrt( r_TOD(1)^2+r_TOD(2)^2 ) );
...
nop = length(swdata);
for i=1:nop
    if ( (year==swdata(1,i)) && (mon==swdata(2,i)) && (day==swdata(3,i)) )
        sw = swdata(:,i);
        break
    end
end
...
[year, mon, day, hour, minute, sec] = invjday (Mjd_UTC-1+2400000.5);
for i=1:nop
    if ( (year==swdata(1,i)) && (mon==swdata(2,i)) && (day==swdata(3,i)) )
        sw_1 = swdata(:,i);
        break
...
[year, mon, day, hour, minute, sec] = invjday(Mjd_UTC-2+2400000.5);
for i=1:nop
    if ( (year==swdata(1,i)) && (mon==swdata(2,i)) && (day==swdata(3,i)) )
        sw_2 = swdata(:,i);
        break
...
[year, mon, day, hour, minute, sec] = invjday(Mjd_UTC-3+2400000.5);
for i=1:nop
    if ( (year==swdata(1,i)) && (mon==swdata(2,i)) && (day==swdata(3,i)) )
        sw_3 = swdata(:,i);
        break
...
[ddum, output.t(2)] = densu(z,1, tinf, tlb, 0, 0,output.t(2),ptm(6), s, mn1, zn1, meso_tn1, meso_tgn1);
