...
UB = [1e-0 0.05e-0 .7e-0 1e-1 2*pi 44*pi/180]; 
LB = [-1e-0 -0.05e-0 -.7e-0 -1e-1 0 10*pi/180];
...
for iii = 1:l_dep
    for jjj = 1:l_TOF
        for kkk = 1:l_nr
            ...
            X_dep = [DepBody.a DepBody.e DepBody.om];
            X_arr = [ArrBody.a ArrBody.e ArrBody.om];
            
            while abs(X_arr(3)-X_dep(3)) > pi
                
                if X_arr(3) > X_dep(3)
                    X_arr(3) = X_arr(3) - 2*pi ;
                else
                    X_arr(3) = X_arr(3) + 2*pi ;
                end
            end
            ...
            fitnessfcn = @(lambda) hill_shaping_DV_2(lambda,departure_elements,arrival_elements,X_dep,X_arr,TOF(jjj),mu_S,shape_flag,param_flag,du_i_h,du_others,0);
%         keyboard
