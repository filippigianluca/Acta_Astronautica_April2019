function [dv,vin,error,xp2,vp2,output] = mimga3_3(t0, T, s, multirev, v0, options, sys)
...
%
%   [dv, vout, error, vbrake, xp2, vp2] = mimga3_3(t0, T, s, v0, options,
%                                                  pla_sys)
%
% INPUT
...
%                   with departure velocity v0. No swing-by is performed at
%                   body s(1).
%               1:  The journey starts with a swing-by at planet s(1). v0
%                   is the incoming velocity before the swing-by manoeuvre.
...
%               1:  The function ends after a swing-by of the last planet,
%                   s(end).
...
%                   conjunction with alpha1=0, to create a trajectory in
%                   which there is no real DSM in the first leg. In fact,
%                   the DSM is moved to emulate launch. Note that in this
%                   case, the first element of the output dv will still be
%                   considered as a DSM.
...
%   pla_sys Optional. A structure which defines the planetary system and
%           ephemerides to use:
...
%           (astro_constants and EphSS).
%
% OUTPUT
%   dv      Vector of DSM dv [km/s]. No launch and no brake dv is computed.
...
%
% CALLED FUNCTIONS
%   lambertMR, swingby, keppro3, astro_constants, d2b_eq,
%   cart2kep.
%   If sys is specified, ephemerides functions pointed by sys.eph and
%   sys.eph_kep; otherwise, EphSS and astro_constants.
%
...
%           of time.
% Modified: Matteo Ceriotti, 01-08-2008: Added the output structure as an
%           output.
% Modified: Matteo Ceriotti, 19-12-2008: Changed two_body_dynamics to
%           d2b_eq.
% Modified: Matteo Ceriotti, 23/02/2009: Fixed empty swingby field in
%           output structure when no swing-by at beginning is performed.
%           Added rp in swingby in output structure. Added body and
%           relative velocity in output structure, leg(1).part(1) when
%           alpha(1)==0. Added dimensional rp and hp.
...
if isempty(sys)
    sys.mu = astro_constants(4); % Planetary constant of the Sun
    sys.mu_planets = astro_constants(11:19);  % Default: Solar System
    sys.R_planets = astro_constants(21:29); % Default: Solar System
    sys.eph_handle = @EphSS; % Default: Solar System
...
while Tindex<=length(T)
    ...
    if sindex==2 && options(1)==0 % sindex==2 means that we are at the first planet
        ...
        if options(5) && nargout > 4 % Output requested
            ...
            output.swingby(indexswingby).incoming.kep = cart2kep([xp1, vin], sys.mu);
            ...
            output.swingby(indexswingby).outgoing.kep = cart2kep([xp1, vout], sys.mu);
    ...
    if sindex>length(s) % sindex>length(s) means that we are at the last planet
        ...
        % Propagation
        [sds,error]=keppro3([xp1,vout],tof*alpha*86400,sys.mu); % Keplerian propagation
        ...
        if options(4)==2 % Verbose
            ...
            temp1 = cart2kep([xp1,vout], sys.mu); % Orbital parameters just after the beginning of the leg
            temp2 = cart2kep(sds, sys.mu); % Orbital parameters just before the DSM
            ...
            temp1 = cart2kep([xds, vds2], sys.mu); % Orbital parameters just after the DSM
            temp2 = cart2kep([xp2, vin], sys.mu); % Orbital parameters just before the end of the leg
        ...
        if options(4) % Plot flag
            ...
            if alpha>0
                ...
                [tsp, ysp]=ode45(@d2b_eq, [0, tof*alpha*86400],[xp1,vout],ode45options, sys.mu); %#ok
                line(ysp(:,1), ysp(:,2), ysp(:,3), 'Color', 'k');
            end
            %[tsp, ysp]=ode45(@two_body_dynamics,[0, tof*(1-alpha)*86400],[xds,vds2],ode45options, sys.mu); %#ok
            [tsp, ysp]=ode45(@d2b_eq,[0, tof*(1-alpha)*86400],[xds,vds2],ode45options, sys.mu); %#ok
