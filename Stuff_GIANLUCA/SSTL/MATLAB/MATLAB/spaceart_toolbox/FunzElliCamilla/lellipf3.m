function f = lellipf3(phi,m,errtol)
%
...
if any(logi_a)
    f(logi_a) = lellipf(phi(logi_a),k,errtol);
end
if any(logi_b|logi_c|logi_d)
    ...
    if any(logi_b)
        ...
        n = fix(phisup/(pi));
        esup = 2*n*Df+lellipf(phisup-n*pi,k,errtol);
        f(logi_b) = esup+sign(phi(logi_b))*4*Df;
    end
    if any(logi_c)
        n = fix(phi(logi_c)/(pi));
        f(logi_c) = 2*n*Df+lellipf(phi(logi_c)-n*pi,k,errtol);
    end
    if any(logi_d)
        n = fix(phi(logi_d)/(pi));
