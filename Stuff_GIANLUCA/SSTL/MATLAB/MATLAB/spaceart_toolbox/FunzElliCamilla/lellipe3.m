function e = lellipe3(phi,m,errtol)
%
...
if any(logi_a)
    e(logi_a) = lellipe(phi(logi_a),k,errtol);
end
if any(logi_b|logi_c|logi_d)
    ...
    if any(logi_b)
        ...
        n = fix(phisup/(pi));
        esup = 2*n*De+lellipe(phisup-n*pi,k,errtol);
        e(logi_b) = esup+sign(phi(logi_b))*4*lellipe(pi/2,k,errtol);
    end
    if any(logi_c)
        n = fix(phi(logi_c)/(pi));
        e(logi_c) = 2*n*De+lellipe(phi(logi_c)-n*pi,k,errtol);
    end
    if any(logi_d)
        n = fix(phi(logi_d)/(pi));
