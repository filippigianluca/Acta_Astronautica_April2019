% =========================================================================
% Optimal thrusting angles strategy for change in the iRAAN.
% Elevation equal to pi/2 and azimuth equal to zero. Elevation change sign
% at antinodes crossing
% Both numerical and averaged analytical integration are realized in this
% file.
% % Numerical integration is stopped when final right ascension is reached. 
% Averaged analytical propagation is realised with FABLE, with two thrust
% arcs appropriately centered along the orbit, with opposite elevation. 
% There are some differences between numerical and analtyical propagation.
% =========================================================================
...
initial_orbit.e     = 0.01;
initial_orbit.i     = 10*pi/180;
initial_orbit.omega = 30*pi/180;
initial_orbit.Omega = 10*pi/180;
initial_orbit.E     = 180*pi/180;



% Final orbit [km, rad]
final_orbit.Omega = 5*pi/180;
...
[Delta_V_analytic, T_Analytic, Kep_Analytic] = RAAN_adjustment(initial_orbit, final_orbit, time_int, engine, m0, element_target, k_p, k_a, constants);


Delta_V_analytic_eq = pi/2 * sqrt(constants.mu_dim / initial_orbit.a) * ...
    abs(final_orbit.Omega - initial_orbit.Omega) * sin(initial_orbit.i);
...
subplot(2,3,6)
plot(Time/ 86400, mod(Kep_Elements(:,5),2*pi)*180/pi,'LineWidth',2)
grid on
xlim([0 Time(end)/86400])
xlabel('Time [days]')
ylabel('\omega [deg]')  
subplot(2,3,5)
plot(Time/ 86400, mod(Kep_Elements(:,4),2*pi)*180/pi,'LineWidth',2)
grid on
xlim([0 Time(end)/86400])
xlabel('Time [days]')
ylabel('\Omega [deg]')
