function [Delta_V, T_Analytic, Kep_Analytic] = inclination_adjustment(initial_orbit, final_orbit, time_int, engine, m0, element_target, k_p, k_a, constants)

% Function for the variation of the inclination using the optimal
...
eta    = -(initial_orbit.omega) * ones(n,1);
csi    = zeros(n,1);
u_ratio = zeros(n,1);
ts =   linspace(0,ToF,n)';
t_wait = 0;
...
flag_ecl = 0;
steps = 3000;
options_ODE = odeset('AbsTol',1e-12,'RelTol',1e-12,'events',@(t,x)events_stop_ode_eq(t,x,element_target,final_orbit.i));
