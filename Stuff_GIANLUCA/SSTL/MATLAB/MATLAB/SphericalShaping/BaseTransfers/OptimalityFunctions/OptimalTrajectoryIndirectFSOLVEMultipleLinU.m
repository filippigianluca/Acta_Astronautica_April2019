function [r,v,lambda,uVec,tVec] = OptimalTrajectoryIndirectFSOLVEMultipleLinU(timeVec,t0,tf,r0,rf,v0,vf,R,Vcart,lambdaAd,M,nT,mu,uMax)
% Function that calculates the optimal trajectory of the base transfer to
% check the results of the spherical shaping method

% Method: indirect multiple shooting with FSOLVE with linear U formualtion
...
optionsFS = optimoptions('fsolve','Algorithm','levenberg-marquardt','Display','iter','TolFun',1e-6,'TolX',1e-5,'MaxFunEvals',100000,'MaxIter',4);
[xSol,fval] = fsolve(@(y) ShootingFunctionFSOLVEMultipleLinU(y,inCond,fCond,mu,timeInt,M,nT,uMax),x0,optionsFS);
...
for i=1:M 
    ...
    [tOde,Y] = rk4(@diffEqLinU,timeInteg,xSol(:,i),1,mu,uMax);
    Y = Y';
