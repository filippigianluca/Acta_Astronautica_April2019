function [r,v,lambda,tVec] = OptimalTrajectoryIndirectFSOLVEMultipleScaled(timeVec,t0,tf,r0,rf,v0,vf,R,Vcart,lambdaAd,M,nT,mu)
% Function that calculates the optimal trajectory of the base transfer to
% check the results of the spherical shaping method

% Method: indirect multiple shooting with FSOLVE, version with scaled equations
...
optionsFS = optimoptions('fsolve','Algorithm','levenberg-marquardt','Display','iter','TolFun',1e-4,'TolX',1e-4,'MaxFunEvals',5000,'MaxIter',5);
[xSol,fval] = fsolve(@(y) ShootingFunctionFSOLVEMultipleScaled(y,inCond,fCond,mu,timeInt,M,nT,lambdaS),x0,optionsFS);
...
for i=1:M
    ...
    [tOde,Y] = rk4(@diffEqScaled,timeInteg,xSol(:,i),1,mu,norm(r0),norm(v0),lambdaS(1),lambdaS(2),tf);
    Y = Y';
...
% hold on;
% plot(timeInt(1:end-1),rInt(1,1:end-1),'go','linewidth',2);
% plot(timeInt(1:end-1),rSol(1,1:end),'ro','linewidth',2);
% ylabel('R x');
% legend('Spherical Shaping','BEFORE-OptVar','AFTER-OptVar','location','northeast');
% subplot(3,1,2);
% plot(timeVec,R(2,:),'b','linewidth',2);
% hold on;
% plot(timeInt(1:end-1),rInt(2,1:end-1),'go','linewidth',2);
% plot(timeInt(1:end-1),rSol(2,1:end),'ro','linewidth',2);
% ylabel('R y');
% subplot(3,1,3);
% plot(timeVec,R(3,:),'b','linewidth',2);
% hold on;
% plot(timeInt(1:end-1),rInt(3,1:end-1),'go','linewidth',2);
% plot(timeInt(1:end-1),rSol(3,1:end),'ro','linewidth',2);
% ylabel('R z');
% suptitle('Radius vector comparison Opt Control-Spherical Shaping');
% xlabel('Time');
