% ========================================================================
% Main script for propagation of orbit in an Earth-centered reference system 
% using averaged analytical propagator. 
% Perturbations that can be included:
% - J2, J3, J4, J5
% - Drag (exponential atomospheric model)
...
% - Low-thrust acceleration with alpha and beta angles constants in the rth
% reference frame and magnitude eps = eps/r^2
% =========================================================================
...
constants.mu_Moon = astro_constants(20) / constants.DU^3 * constants.TU^2;
...
% J2, J3, J4, J5
constants.J2 =  1082.63e-6;
constants.J3 =  -2.5327e-6;
constants.J4 =  -1.6196e-6;
constants.J5 =  -2.2730e-7;
...
% Area to mass ratio for Solar Radiation Pressure [m^2/kg]
A_m_SRP = 0.05;
...
beta_rth  = 10*pi/180;
alpha_rth = pi/2+0*15*pi/180;


% ------------------------------------------------------------------
% Constant acc with eps = 1/r2
flag_rth_r2 =1;
% Elevation and azimuth angle of the acceleration [rad]
beta_rth_r2  = 30*pi/180;
alpha_rth_r2 = pi/2+0*15*pi/180;
...
dL_a   = flag_peri_apo * 45*pi/180 * ones(n,1);
dL_p   = flag_peri_apo * 45*pi/180 * ones(n,1);
% Elevation of the perigee and apogee thrust arc
beta_a = 0*flag_peri_apo * 0*-pi/2 * ones(n,1);
beta_p = 0*flag_peri_apo * 0*pi/2 * ones(n,1);
...
% -------------------------------------------------------------------------
% stop_eccentricity_value = 0.05;
stop_altitude_value     = 300;

% -------------------------------------------------------------------------
% Options for numerical integration
% -------------------------------------------------------------------------
options_ODE_NUM = odeset('AbsTol',1e-12,'RelTol',1e-12);
...
elseif stop_eccentricity
    options_ODE = odeset('AbsTol',1e-7,'RelTol',1e-7,'events',@(t,x)events_reentry(t,x,stop_eccentricity_value,'eccentricity'));
end
...
if num_comparison
    ...
    [T_Numeric, Equin_Numeric] = ode45(@(t,x)equations_motion3(t, x, ...
        input_num_int, thrust, control, drag, constants.g0),...
        linspace(0, T_Analytic(end), steps_num*steps),...
...
if num_comparison
    ...
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU / constants.sec_day, h_perigee_Analytic * constants.DU,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ...
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1))*constants.TU/constants.sec_day, h_apogee_Analytic*constants.DU,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ...
    subplot(2,2,1)
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1))*constants.TU/constants.sec_day, Equin_Analytic(:,1)*constants.DU,'b')
    hold on
    ...
    subplot(2,2,2)
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1))*constants.TU / constants.sec_day, Equin_Analytic(:,2),'b')
    hold on
    ...
    subplot(2,2,3)
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, Equin_Analytic(:,3),'b')
    hold on
    ...
    subplot(2,2,4)
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, Equin_Analytic(:,4),'b')
    hold on
    ...
    figure
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, Equin_Analytic(:,5),'b')
    hold on
    ...
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1))*constants.TU /constants.sec_day, e_Analytic,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ylabel('e')
    legend('Numerical','Averaged analytical')
    ...
    subplot(2,2,1)
       plot((T_Numeric(1:steps_num:end)-T_Numeric(1))*constants.TU / constants.sec_day,Equin_Numeric(1:steps_num:end,1)*constants.DU,'r','LineWidth',2)
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1))*constants.TU/constants.sec_day, Equin_Analytic(:,1)*constants.DU,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ylabel('a [km]')
...
%     legend('Numerical','Averaged analytical')
    ...
    plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day, incl_Numeric*180/pi,'r','LineWidth',2)
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, incl_Analytic*180/pi,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ylabel('i [deg]')
        axhand=gca;
...
%     legend('Numerical','Averaged analytical')
    ...
    plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day, Keplerian_Numeric(4,:)*180/pi,'r','LineWidth',2)
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, Keplerian_Analytic(4,:)*180/pi,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ylabel('\Omega [deg]')
        axhand=gca;
...
plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day, Keplerian_Numeric(5,:)*180/pi,'r','LineWidth',2)
    hold on
    plot((T_Analytic(1:steps_num:end)-T_Analytic(1)) * constants.TU/constants.sec_day, Keplerian_Analytic(5,:)*180/pi,'b','LineWidth',2)
    grid on
    xlabel('Time [days]')
    ylabel('\omega [deg]')
        axhand=gca;
...
%     legend('Numerical','Averaged analytical')
   ...
   plot((T_Numeric(1:steps_num:end)-T_Numeric(1))*constants.TU /constants.sec_day, e_Analytic-e_Numeric,'k','LineWidth',2)
    ...
    ylabel('\Delta e')
    subplot(2,3,4)
   plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day,...
       (incl_Analytic-incl_Numeric)*180/pi,'k','LineWidth',2)
    xlabel('Time [days]')
    ylabel('\Delta i [deg]')
    subplot(2,3,6)
    plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day, ...
        (Keplerian_Analytic(4,:)-Keplerian_Numeric(4,:))*180/pi,'k','LineWidth',2)
    xlabel('Time [days]')
    ylabel('\Delta \Omega [deg]')
    subplot(2,3,5)
 plot((T_Numeric(1:steps_num:end)-T_Numeric(1)) * constants.TU/constants.sec_day, ...
     (Keplerian_Analytic(5,:)-Keplerian_Numeric(5,:))*180/pi,'k','LineWidth',2)
    xlabel('Time [days]')
    ylabel('\Delta \omega [deg]')
    ...
    subplot(2,3,1)
    hold on
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Numeric(:,1)* constants.DU,'LineWidth',2,'Color','r')
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Analytic(:,1) * constants.DU,'LineWidth',2,'Color','b')
    grid on
    xlabel('Time [days]')
    ylabel('a [km]')
%     legend('Numerical Propagation','Averaged Analytical Propagation')
    axhand=gca;
    set(axhand,'FontSize',16)
    subplot(2,3,2)
    hold on
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Numeric(:,2),'LineWidth',2,'Color','r')
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Analytic(:,2),'LineWidth',2,'Color','b')
    grid on
    xlabel('Time [days]')
    ylabel('P_1')
    legend('Numeric propagator','Analytic propagator')
    axhand=gca;
    set(axhand,'FontSize',16)
    subplot(2,3,3)
    hold on
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Numeric(:,3),'LineWidth',2,'Color','r')
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day,...
        Equin_Analytic(:,3),'LineWidth',2,'Color','b')
    grid on
    xlabel('Time [days]')
    ylabel('P_2')
        axhand=gca;
    ...
    hold on
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Numeric(:,5),'LineWidth',2,'Color','r')
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day,...
        Equin_Analytic(:,5),'LineWidth',2,'Color','b')
    grid on
    xlabel('Time [days]')
    ylabel('Q_2')
        axhand=gca;
    ...
    hold on
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Numeric(:,4),'LineWidth',2,'Color','r')
    plot((T_Analytic(:)-T_Analytic(1))*constants.TU/constants.sec_day, ...
        Equin_Analytic(:,4),'LineWidth',2,'Color','b')
    grid on
    xlabel('Time [days]')
    ylabel('Q_1')
%     set(gcf,'NextPlot','add');
