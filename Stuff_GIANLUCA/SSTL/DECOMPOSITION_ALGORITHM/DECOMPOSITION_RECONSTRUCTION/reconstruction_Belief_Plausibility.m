function [decomposition_end, Plot_decomposition, num_sample_tot, LIST] = reconstruction_Belief_Plausibility(in, problem_decomposition, minmax, minmin, Sample, n_obj, n_point, algo_decomposition)
...
num_coupled_vectors = length(in.dim_u(in.num_functions +1 : end)); 
...
if in.output == 0 % evaluate only Belief
    ...
    for index_num_coupled_vectors = 1:num_coupled_vectors
        if in.dim_u(in.num_functions + index_num_coupled_vectors)>0
...
%     problem_inner = build_metaproblem_ideaminmax_s_inner(problem_decomposition);
    ...
    problem_inner = build_problem_inner(problem_decomposition, minmax, minmin, n_obj);
    ...
    [decomposition_end] = evaluate_combination_FE_decoupled(decomposition_end, num_sample, problem_decomposition, minmax, minmin, u_max_tot, Sample, PS, u_max_tot_Plausibility, n_obj, n_point, in);
...
if in.output == 0
    ...
    figure
    stairs([LIST.F_Bel(1) LIST.F_Bel], [0 LIST.Bel],'linewidth',2)
    xlabel('F')
    ylabel('Belief ')
