%% ========================================================================
% TEST FILE FOR OSCULATING ELEMENTS PROPAGATION USING:
% - J2
% - Atmospheric drag
...
addpath(genpath('FunzElliCamilla'))
...
% Mass unit 
MU = astro_constants(13) / astro_constants(1);
...
% Rectification every one orbit
old = 0;

% No rectification at all
new = 1;
...
% kep0 = [semimajor eccentricity 30*(pi/180) 60*(pi/180) 0 0];
% kep0 = [1.12 0.05 0*(pi/180) 0*(180/pi) 0 0];
% kep0 = [1.13 0.05 30*(pi/180) 60*(180/pi) 0 0*pi/180];

% Esempio paper Federico
% kep0 = [1.1759 0.1 6*(pi/180) 0 10*(pi/180) 0];
% kep0 = [1.2273 0.0064 6*(pi/180) 0 10*(pi/180) 0*pi/180];
% kep0 = [1.1921 0.1019 30*(pi/180) 60*(pi/180) 120*pi/180 0];
% kep0 = [1.11 0.05 6*(pi/180) 0 10*(pi/180) 0];
% kep0 = [semimajor eccentricity 0*(pi/180) 0*(180/pi) 0 0];

% kep0 = [1.2470 0.1791 0*(pi/180) 0 0*(pi/180) 0];

% kep0 = [3.8378 0.73 0*(pi/180) 0*(180/pi) 0*180/pi 0];


% kep0 = [(301+DU)/DU 0.01 0*(pi/180) 0*(pi/180) 0*(pi/180) 0];
kep0 = [(500+DU)/DU 0.01 30*pi/180 0 30*pi/180 0];
% kep0 = [(800+DU)/DU 0.0 0*pi/180 0 0*pi/180 0];

% kep0 = [1.1686 0.1240 0 0 0 0];
...
% Put the folllowing flag to 1 to include any of the following
% perturbations:
...
if old == 1
    ...
    %     OLD
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     For cycle over the number of revolution (because after each revolution the
%     atmospheric density needs to be updated - this was the old reason. Now it 
%     can be seen just as a rectification after every orbit)
    tic
    
    for k = 1 : rev
        
        % At the beginning the initial condition are the same for both
        % integration
        if k == 1
            Equin0_AN = Equin0;
            Equin0_NUM = Equin0';
        ...
        L = linspace(Equin0_AN(6), Equin0_AN(6) + 2*pi, n_points);
        ...
        [Equin,t] = AnEquin_all_forward_tang_m(L, Equin0_AN, epsilon_t, beta_t,epsilon_rth ,alpha_rth ,beta_rth ,epsilon_In ,alpha_In ,beta_In , mu, J2, 1, drag, 0);
        % ---------------------------------------------------------------------
        ...
        % ---------------------------------------------------------------------
        % Analytical propagation
        EquinoctialElements = [EquinoctialElements Equin];
        if k == 1
            time = [time t];
        else
        ...
        % Define new condition for next propagation only if more than one
        % revolution is considered and if we are currently not in the last
        % revolution
        if rev > 1  && k < rev
            % ---------------------------------------------------------------------
            ...
            % -----------------------------------------------------------------
            % Update density - Analytical propagation
            % -----------------------------------------------------------------
            % Keplerian elements of the final position
            kep_final = eq2kep(Equin(:,end));

            % -----------------------------------------------------------------
            % Check if spacecraft has re-entered
            % -----------------------------------------------------------------
            if h_p < 150 / DU
                disp('Re-entered spacecraft')
                EquinoctialElementsNUM = [EquinoctialElementsNUM; [EquinNUM(end,1:5) L(end,1)]];
                break
            end
...
elseif new == 1
    ...
    t0 = 0;
    % Propagate for one orbit
    L = linspace(Equin0_AN(6), Equin0_AN(6) + 2*pi*rev, n_points*rev-rev+1);
    
    % [Equin,t] = AnEquin_all_forward_tang_m(L, Equin0, epsilon_t, beta_t, epsilon_rth, alpha_rth, beta_rth, epsilon_In, alpha_In, beta_In, muadim, J2, R, ill_flag)
    [Equin,t] = AnEquin_all_forward_tang_m(L, Equin0_AN, epsilon_t, beta_t,epsilon_rth ,alpha_rth ,beta_rth ,epsilon_In ,alpha_In ,beta_In , mu, geopotential, third_body, 1, drag, 0);
    toc
...
for k = 1 : size(EquinoctialElements,2)
    ...
    cartesian_elements(:,k) = kep2cart(keplerian_elements(:,k),mu);
    cartesian_elements_NUM(:,k) = kep2cart(keplerian_elements_NUM(:,k),mu);
    
    h_a(k) = keplerian_elements(1,k) * (1 + keplerian_elements(2,k)) - R;
    h_p(k) = keplerian_elements(1,k) * (1 - keplerian_elements(2,k)) - R;
    
    h_a_NUM(k) = keplerian_elements_NUM(1,k) * (1 + keplerian_elements_NUM(2,k)) - R;
    h_p_NUM(k) = keplerian_elements_NUM(1,k) * (1 - keplerian_elements_NUM(2,k)) - R;
    
    
end

height = keplerian_elements(1,:) .* (1-keplerian_elements(2,:).^2) ./ (1+keplerian_elements(2,:) .* cos(keplerian_elements(6,:))) - R;
height_NUM = keplerian_elements_NUM(1,:) .* (1-keplerian_elements_NUM(2,:).^2) ./ (1+keplerian_elements_NUM(2,:) .* cos(keplerian_elements_NUM(6,:)))  - R;
...
%     plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), abs(delta(1,:))*DU)
    plot(linspace(0,rev,rev*n_points-rev+1), abs(delta(1,:))*DU)
    xlabel('Revolutions')
    ylabel('\Delta a [km]')
    grid on
    subplot(2,2,2)
%         plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), abs(delta(2,:)))
    plot(linspace(0,rev,rev*n_points-rev+1), abs(delta(2,:)))
    xlabel('Revolutions')
    ylabel('\Delta P_1')
    grid on
    subplot(2,2,3)
%         plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), abs(delta(4,:)))
    plot(linspace(0,rev,rev*n_points-rev+1), abs(delta(4,:)))
    xlabel('Revolutions')
    ylabel('\Delta Q_1')
    grid on
    subplot(2,2,4)
%         plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), abs(delta_time * TU ))
    plot(linspace(0,rev,rev*n_points-rev+1), abs(delta_time * TU ))
    xlabel('Revolutions')
    ylabel('\Delta T [s]')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), (delta(1,:))*DU)
    xlabel('Arclength [rad]')
    ylabel('\Delta a [km]')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), delta(2,:))
    xlabel('Arclength [rad]')
    ylabel('\Delta P_1')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), delta(3,:))
    xlabel('Arclength [rad]')
    ylabel('\Delta P_2')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), delta(4,:))
    xlabel('Arclength [rad]')
    ylabel('\Delta Q_1')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), delta(5,:))
    xlabel('Arclength [rad]')
    ylabel('\Delta Q_2')
    grid on
    ...
    % ---------------------------------------------------------------------
    figure
    plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)), delta_time * TU )
    xlabel('Arclength [rad]')
    ylabel('\Delta T [s]')
    grid on
    ...
    subplot(1,2,1)
    hold on
    plot(linspace(0,rev,rev*n_points-rev+1),keplerian_elements(1,:)*DU,'b','LineWidth',2)
    hold on
    plot(linspace(0,rev,rev*n_points-rev+1),keplerian_elements_NUM(1,:)*DU,'r','LineWidth',2)
    legend('Analytical Propagation','Numerical Propagation')
    xlabel('Revolutions')
    ylabel('a [km]')
    subplot(1,2,2)
    hold on
    plot(linspace(0,rev,rev*n_points-rev+1),keplerian_elements(2,:),'b','LineWidth',2)
    hold on
    plot(linspace(0,rev,rev*n_points-rev+1),keplerian_elements_NUM(2,:),'r','LineWidth',2)
    legend('Analytical Propagation','Numerical Propagation')
    xlabel('Revolutions')
    ylabel('e')
...
diff_position = norm(cartesian_elements(1:3,end) - cartesian_elements_NUM(1:3,end)) * DU * 1000;
diff_velocity = norm(cartesian_elements(4:6,end) - cartesian_elements_NUM(4:6,end)) * DU * 1000 /(TU);
diff_time = delta_time(end)*TU;

string1 = strcat('Final position error is [m]: ',num2str(diff_position));
string2 = strcat('Final velocity error is [m/s]: ',num2str(diff_velocity));
string3 = strcat('Final time error is [s]: ',num2str(diff_time));
disp(string1)
disp(string2)
disp(string3)

disp(strcat('Final semimajor axis error is [m]: ',num2str(delta_kep(1,end)*DU*1000)))
disp(strcat('Final eccentricity error is : ',num2str(delta_kep(2,end))))
disp(strcat('Final inclination error is [deg] : ',num2str(delta_kep(3,end)*180/pi)))
disp(strcat('Final RAAN error is [deg] : ',num2str(delta_kep(4,end)*180/pi)))
disp(strcat('Final perigee argument error is [deg] : ',num2str(delta_kep(5,end)*180/pi)))
...
% figure
% % subplot(1,2,1)
% plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),h_p_NUM*DU,'g','LineWidth',2)
% hold on
% plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),h_a_NUM*DU,'r','LineWidth',2)
% for i = 1 : length(h_crossing)
%     line([0 2*pi],[h_crossing(i)*DU h_crossing(i)*DU])
% end
% 
% subplot(1,2,2)
% plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),h_p*DU,'g','LineWidth',2)
% hold on
% plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),h_a*DU,'r','LineWidth',2)
% for i = 1 : length(h_crossing)
%     line([0 2*pi],[h_crossing(i)*DU h_crossing(i)*DU])
% end
% 
...
% 
% figure
% plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),height*DU,'b','LineWidth',2)
% hold on
% grid on
% % plot(linspace(0,rev*2*pi,size(EquinoctialElements,2)),height_NUM,'r','LineWidth',2)
% % for i = 1 : length(h_crossing)
% %     line([0 2*pi],[h_crossing(i) h_crossing(i)])
% % end
...
% props.Cdata = topo;
% h=surface(xs,ys,zs,props);
% hold on
% plot3(cartesian_elements(1,:),cartesian_elements(2,:),cartesian_elements(3,:),'k','LineWidth',2)
% hold on
% grid on
...
% h=surface(xs,ys,zs,props);
% hold on
% plot3(cartesian_elements_NUM(1,:),cartesian_elements_NUM(2,:),cartesian_elements_NUM(3,:),'k','LineWidth',2)
% hold on
% plot(0,0,'ko','MarkerFaceColor','k')
% grid on
% axis square
...
delta_a= J2 * R^2 / kep0(1) * ( (kep0(1)./r).^3 - 1 /(1-e^2)^(3/2) + 1.5 .* sin(kep0(3))^2 .* ...
    (-(kep0(1)./r).^3 +  1 /(1-e^2)^(3/2) + (kep0(1)./r).^3 .* cos(2*kep0(5)+2*true_anomaly) ) );


delta_e = J2 * R^2 / 4 * (-2/(kep0(1)^2 * e * sqrt(1-e^2)) + 2 * a * (1-e^2) ./ (e * r.^3) + sin(kep0(3))^2 * (3 /(a^2 * e * sqrt(1-e^2)) - 3 *a * (1-e^2) ./ (e * r.^3)+...
...
figure
plot(true_anomaly, (kep0(1) + delta_a)*DU,'LineWidth',2)
hold on
plot(true_anomaly, kep0(1)*DU*ones(1,length(delta_a)),'k','LineWidth',2)
xlabel('\theta [rad]')
ylabel('a [km]')
grid on
