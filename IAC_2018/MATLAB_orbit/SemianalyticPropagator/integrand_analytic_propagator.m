function [dx_dt, t_f, x_f] = integrand_analytic_propagator(t, x, control, input, constants)                                 
...
%     tii(ii) = (abs(t)>=ts(ii) && abs(t)<ts(ii+1));
% end
% tii = [tii, 0];
% % t12 = (abs(t)>=ts(1) && abs(t)<ts(2));
% % t23 = (abs(t)>=ts(2) && abs(t)<ts(3));
% % t34 = (abs(t)>=ts(3) && abs(t)<ts(4));
% % t4 = (abs(t)>=ts(4));
% 
% dL_a_i = dot(dL_a,tii) ;
% dL_p_i = dot(dL_p, tii) ; 
% beta_a_i = dot(beta_a, tii) ;
% beta_p_i = dot(beta_p, tii) ;
% eta_i = dot(eta, tii) ;
% csi_i = dot(csi, tii) ;
% u_ratio_i = dot(u_ratio, tii) ;
...
% % COMMENT BELOW AND UNCOMMENT beta_p_i AND beta_a_i ABOVE
% e = sqrt(Eq_a_o(2)^2+Eq_a_o(3)^2);
% RAAN_omega = atan2(Eq_a_o(2), Eq_a_o(3));
% RAAN = atan2(Eq_a_o(4), Eq_a_o(5));
% omega = mod(RAAN_omega - RAAN, 2*pi);
% beta_a_i = beta_a_i * sign(cos(omega));
% beta_p_i = - beta_p_i * sign(cos(omega));

% eta_i = mod(mod(asin(-Eq_a_o(2) .* sin(omega)),2*pi) - omega,2*pi);
% eta_i = asin(-e .* sin(omega)) - omega;
...
if ecl_flag || dL_p_i || dL_a_i

    
    [Eq_end_J2, dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) Eq_a_o(6) + 2*pi], Eq_a_o, ...
        0, 0, 0, ...
        0, 0, 0, ...
        alpha_Sun, beta_Sun, muadim, geopotential, third_body, R, drag, Earth_flat, ill_flag); % Coasting leg
    
    
%     [Eq_end_J2, dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) Eq_a_o(6) + 2*pi], Eq_a_o, ...
%         0, 0, 0, ...
%         0, 0, 0, ...
%         0, 0, muadim, geopotential, third_body, R, drag, Earth_flat, ill_flag);
%     
    ...
    % Do not use drag and third body for the rest of the propagation
    drag.CD = 0;
    third_body.flag_Moon = 0;
    third_body.flag_Sun = 0;
    ...
    if (L_o > L_p_i) 
        ...
        if L_i> Eq_a_o(6)
            ...
            [Eq_a_o, dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) L_i], Eq_a_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth, ...
                                                       ...
                                                       thrust.rth_flag_eps_r2); % Coasting leg
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Update mass:
...
%             m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
    ...
    % If the eclipse exit point is after the end of the post-apogee coasting arc, propagate the coasting, then the eclipse, and finally another coasting arc.
        ...
        if L_i > Eq_a_o(6)
            ...
            [Eq_a_o,dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) L_i], Eq_a_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth, ...
                                                      T_Sun_adim/m, alpha_Sun, beta_Sun, muadim, geopotential, third_body, R, drag, Earth_flat, ill_flag, ...
                                                      thrust.rth_flag_eps_r2); % 1st Coasting leg
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            DT = DT + dt;
            % Update mass:
%             m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
        ...
        [Eq_a_o,dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) L_o], Eq_a_o, 0, 0, 0, 0, 0, 0, 0, 0, muadim, geopotential, third_body, R, drag, Earth_flat, ill_flag, ...
            thrust.rth_flag_eps_r2); % Eclipse leg
        DT = DT + dt;
        ...
        [Eq_p_i,dt] = AnEquin_all_forward_tang_m([Eq_a_o(6) L_p_i], Eq_a_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth, ...
                                                 T_Sun_adim/m, alpha_Sun, beta_Sun, muadim, geopotential, third_body, R, drag, Earth_flat, ill_flag, ...
                                                 thrust.rth_flag_eps_r2); % 2nd Coasting leg
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        DT = DT + dt;
        % Update mass:
%         m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
...
%         save(['Escape_reached_' num2str(round(100*rand))])
        ...
        [dL_i,fval,exitflag] = fsolve(@(x)match_Eq_e(x,Eq_p_i,k_p*epsilon,beta_p_i,u_ratio_i,...
            T_Sun_adim/m,alpha_Sun,beta_Sun,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag,0.98,n_FPET,thrust.rth_flag_eps_r),0,options);
        ...
        if exitflag<1  
            [Eq_f, dt]=AnEquin_all_forward_tang_om_change_in_thrust_multi_m2([Eq_p_i(6) Eq_p_i(6)+dL_i],Eq_p_i,k_p*epsilon,beta_p_i,u_ratio_i,0,0,0,T_Sun_adim/m,alpha_Sun,beta_Sun,...
                muadim,geopotential, third_body, R,drag, Earth_flat, ill_flag,n_FPET, thrust.rth_flag_eps_r);
            Eq_f=Eq_p_i;
        else
            [Eq_f, dt]=AnEquin_all_forward_tang_om_change_in_thrust_multi_m2([Eq_p_i(6) Eq_p_i(6)+dL_i],Eq_p_i,k_p*epsilon,beta_p_i,u_ratio_i,0,0,0,T_Sun_adim/m,alpha_Sun,beta_Sun,...
                muadim,geopotential, third_body, R,drag,Earth_flat, ill_flag,n_FPET, thrust.rth_flag_eps_r);
...
m = m * exp(-abs(k_p)*epsilon * dt_p * m_rate);

% REMOVE THIS!!!!!!!!!!!!!!!!!!!!
% m = m - T_adim /( thrust.Isp * g0) * dt_p;
    ...
    if(L_o > L_a_i) 
        ...
        if L_i > Eq_p_o(6)
            ...
            [Eq_p_o,dt] = AnEquin_all_forward_tang_m([Eq_p_o(6) L_i], Eq_p_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth,...
                                                     T_Sun_adim/m,alpha_Sun,beta_Sun,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag, ...
                                                     thrust.rth_flag_eps_r); % Coasting leg
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            DT = DT+dt;
            % Update mass:
%             m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
        ...
        [Eq_a_i,dt] = AnEquin_all_forward_tang_m([Eq_p_o(6) L_a_i],Eq_p_o,0,0,0,0,0,0,0,0,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag, ...
            thrust.rth_flag_eps_r); % Eclipse leg
...
%             keyboard
            ...
            [Eq_p_o,dt]=AnEquin_all_forward_tang_m([Eq_p_o(6) L_i],Eq_p_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth,...
                                                    T_Sun_adim/m,alpha_Sun,beta_Sun,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag, ...
                                                    thrust.rth_flag_eps_r); % 1st Coasting leg
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            DT=DT+dt;
            % Update mass:
%             m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
        ...
        [Eq_p_o,dt]=AnEquin_all_forward_tang_m([Eq_p_o(6) L_o],Eq_p_o,0,0,0,0,0,0,0,0,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag, ...
            thrust.rth_flag_eps_r); % Eclipse leg
...
%         keyboard
        ...
        [Eq_a_i,dt]=AnEquin_all_forward_tang_m([Eq_p_o(6) L_a_i],Eq_p_o, epsilon_t, thrust.beta_t, epsilon_rth, thrust.alpha_rth, thrust.beta_rth,...
                                               T_Sun_adim/m,alpha_Sun,beta_Sun,muadim,geopotential, third_body, R,drag,Earth_flat,ill_flag, ...
                                               thrust.rth_flag_eps_r); % 2nd Coasting leg
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        DT=DT+dt;
        % Update mass:
%         m = m * exp( -(epsilon_t + epsilon_rth) * dt * m_rate);
...
if dL_a_i>0
    ...
    [Eq_f, dt_f]=AnEquin_all_forward_tang_om_change_in_thrust_multi_m2([Eq_a_i(6) Eq_a_i(6)+dL_a_i*2],Eq_a_i,...
                                                                        ...
                                                                        0,0,0,T_Sun_adim/m,alpha_Sun,beta_Sun,...
                                                                        muadim,geopotential,third_body,R,drag,Earth_flat,ill_flag,n_FPET, ...
                                                                        thrust.rth_flag_eps_r);
%     keyboard
    ...
    if ((Eq_f(2)^2+Eq_f(3)^2)>=0.98)
        ...
        [dL_i,fval,exitflag]=fsolve(@(x)match_Eq_e(x,Eq_a_i,k_a*epsilon,beta_a_i,u_ratio_i,...
        T_Sun_adim/m,alpha_Sun,beta_Sun,muadim,geopotential,third_body,R,drag,Earth_flat,ill_flag,0.98,n_FPET,thrust.rth_flag_eps_r),0,options);
...
% Update mass
m = m*exp(-abs(k_a)*epsilon*dt_f*m_rate);
...
%     fprintf('i')
%     [Eq_f(6) L_adj]
    [Eq_f,dt]=AnEquin_all_forward_tang_m([Eq_f(6) L_adj],Eq_f,0,0,0,0,0,0,0,0,muadim,geopotential,third_body,R,drag,Earth_flat,ill_flag,thrust.rth_flag_eps_r);
%     keyboard
    DT=DT+dt;
    t_ecl=t_ecl+t_ecl_switch*dt;
    % -------------------------------------------------------------
...
% Inclination
% i = Eq_i(3);
i = 2 * atan(sqrt(Eq_i(4)^2 + Eq_i(5)^2));
...
function err=match_Eq_e(x,Eq_0,epsilon,beta,u_ratio_i,epsilon_Sun,alpha_Sun,beta_Sun,muadim,geopotential,third_body,R,drag,Earth_flat,ill_flag,e_target,n_FPET,flag_eps_r2)

    [Eq_f, dt]=AnEquin_all_forward_tang_om_change_in_thrust_multi_m2([Eq_0(6) Eq_0(6)+x],Eq_0,epsilon,beta,u_ratio_i,0,0,0,...
        epsilon_Sun,alpha_Sun,beta_Sun,muadim,geopotential,third_body, R,drag,Earth_flat,ill_flag,n_FPET, flag_eps_r2);
