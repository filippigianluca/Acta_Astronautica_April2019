...
if  (a-constant.Rearth) <= 150000  && (a-constant.Rearth) > 100000
    [H,c1,p,knj,kn]=TD88_Constants100to150();
elseif (a-constant.Rearth) <= 750000   && (a-constant.Rearth) > 150000 
...
% Call Parameters
[fo,fx,ko,g1_4,gB,delta,Tau,cj,zj,Bj]=TD88_Parameters(X,solardata,constant,d,t,c1,p,H,S,m);
...
[Uw,Ujw,V5w,V6w,V7w,V5jw,V6jw,V7jw]=Deriv_eDw_low_e(e,I,w,CapitalOmega,Alpha,Tau,cj,zj,p);
...
Da=((-1) * 2 * pi * (a^2) * delta * fx * fo * ko * (DaSum1 + DaSum2)) * NF;
De=((-1) * 2 * pi * a * delta * fx * fo * ko * (DeSum1 + DeSum2)) * NF;
DI=((-1) * 2 * pi * a * delta * fx * fo * ko * (Tau * tan(I) / (2 * (1 - e^2))) * (DISum1 + DISum2)) * NF;
eDw=((-1) * 2 * pi * a * delta * fx * fo * ko * (eDwSum1 + eDwSum2)) * NF;
