function [FunVal] = ShootingFunctionOPTIMultipleConst(y,inCond,fCond,mu,timeInt,M,nT)

% ShootingFunctionOPTIMultiple
...
for i=1:M
  r(:,i) = r_V(3*(i-1)+1:3*(i-1)+3);
  v(:,i) = v_V(3*(i-1)+1:3*(i-1)+3);
  lambdar(:,i) = lambdar_V(3*(i-1)+1:3*(i-1)+3);
  lambdav(:,i) = lambdav_V(3*(i-1)+1:3*(i-1)+3);
end

lambda = [lambdar; lambdav];
...
for i=1:M 
    ...
    % Integration on each sub-interval
     ...
     [tt,Y] = rk4(@diffEq,timeInteg,[r_in;v_in;lambda_in],1,mu);
     Y = Y';
