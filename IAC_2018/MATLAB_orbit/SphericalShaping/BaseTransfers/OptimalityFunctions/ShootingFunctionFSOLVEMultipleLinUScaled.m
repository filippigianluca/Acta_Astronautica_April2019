function [FunVal] = ShootingFunctionFSOLVEMultipleLinUScaled(y,inCond,fCond,mu,timeInt,M,nT,uMax,lambdaS)
...
% lambdaS: initial value of lambda multipliers, for scaling
%
%OUTPUT
...
for i=1:M 
    ...
    %Y = Y';
     ...
     %%% Rk 4, n steps %%%
     timeInteg = linspace(timeInt(i),timeInt(i+1),nT+1);
     [tt,Y] = rk4(@diffEqLinUScaled,timeInteg,y(:,i),1,mu,uMax,norm(r0),norm(v0),lambdaS(1),lambdaS(2),tf);
     Y = Y';
