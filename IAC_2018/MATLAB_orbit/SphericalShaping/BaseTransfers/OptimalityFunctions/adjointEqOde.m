function lambdadot = adjointEqOde(t,lambda,R,timeVec,mu)

%adjointEq: function that writes the adjoints differential equations for
%the optimal control problem (minimum Delta V). Version for ode
%
% INPUT
% t: time
...
if ( mod(index,1) == 0 )
  ...
  % Select the correspondent radius vector
  r = R(:,index);
