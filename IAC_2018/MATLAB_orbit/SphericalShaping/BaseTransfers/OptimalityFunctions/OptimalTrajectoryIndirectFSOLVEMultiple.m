function [r,v,lambda,tVec] = OptimalTrajectoryIndirectFSOLVEMultiple(timeVec,t0,tf,r0,rf,v0,vf,R,Vcart,lambdaAd,M,nT,mu)
% Function that calculates the optimal trajectory of the base transfer to
% check the results of the spherical shaping method

% Method: indirect multiple shooting with FSOLVE
...
% optionsFS = optimoptions('fsolve','Algorithm','levenberg-marquardt','Display','iter','TolFun',1e-2,'TolX',1e-9,'MaxFunEvals',50000,'MaxIter',10);
% [xSol,fval] = fsolve(@(y) ShootingFunctionFSOLVEMultiple(y,inCond,fCond,mu,timeInt,M,nT),x0,optionsFS);

% Pass the initial guess to fminsearch (doesnt work)
%optionsFMS = optimset('Display','iter','TolFun',1e-2,'TolX',1e-9,'MaxFunEvals',50000,'MaxIter',10);
%[xSol] = fminsearch(@(y) ShootingFunctionFSOLVEMultiple(y,inCond,fCond,mu,timeInt,M,nT),x0,optionsFMS);

% Pass the initial guess to fmincon
optionsFMC = optimoptions('fmincon','Algorithm','interior-point','Display','iter','TolX',1e-9,'TolCon',1e-3,'TolFun',1e-4,'MaxFunEvals',100000,'MaxIter',100);
xSol = fmincon(@(y) ShootingFunctionFMINCONObj(y,M,timeInt,nT,mu),x0,[],[],[],[],[],[],@(y) ShootingFunctionFMINCONConst(y,inCond,fCond,mu,timeInt,M,nT),optionsFMC);
...
for i=1:M 
    ...
    [tOde,Y] = rk4(@diffEq,timeInteg,xSol(:,i),1,mu);
    Y = Y';
