function [r,v,u,tVec,exitFlag] = OptimalTrajectoryDirectMat(N,timeVec,t0,tf,r0,rf,v0,vf,R,Vcart,Ucart,mu,uMax)
% Function that calculates the optimal trajectory of the base transfer to
% check the results of the spherical shaping method

% Method: direct transcription solved with fmincon
...
% exitFlag: indicator on the convergence of the algorithm
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%      end
    ...
    % fmincon
    options = optimoptions('fmincon','Algorithm','interior-point','Display','iter-detailed','TolX',1e-9,'TolCon',1e-7,'TolFun',1e-7,'MaxFunEvals',100000,'MaxIter',1000);
    [xSol,fval,exitFlag] = fmincon(@(y) DirTranscFunctionFMINCONMatObj(y,N,tVec),x0,[],[],[],[],[],[],@(y) DirTranscFunctionFMINCONMatConst(y,inCond,fCond,N,h,mu,uMax),options);
...
%      end
    ...
    % Find the correct initial conditions for the lagrange multipliers from
    % fmincon
    options = optimoptions('fmincon','Algorithm','interior-point','Display','iter-detailed','TolX',1e-9,'TolCon',1e-7,'TolFun',1e-7,'MaxFunEvals',200000,'MaxIter',100000);
    [xSol,fval,exitFlag] = fmincon(@(y) DirTranscFunctionFMINCONGLObj(y,N,tVec),x0,[],[],[],[],[],[],@(y) DirTranscFunctionFMINCONGLConst(y,inCond,fCond,N,h,mu,uMax),options);
