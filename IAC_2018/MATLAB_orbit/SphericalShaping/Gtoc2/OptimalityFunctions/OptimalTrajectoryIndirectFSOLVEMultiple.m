function [r,v,lambda,tVec,exitFlag] = OptimalTrajectoryIndirectFSOLVEMultiple(timeVec,t0,tf,r0,rf,v0,vf,R,Vcart,lambdaAd,M,nT,mu)
...
% optionsFS = optimoptions('fsolve','Algorithm','levenberg-marquardt','Display','iter','TolFun',1e-2,'TolX',1e-9,'MaxFunEvals',5000,'MaxIter',10);
% [xSol,fval,exitFlag] = fsolve(@(y) ShootingFunctionFSOLVEMultiple(y,inCond,fCond,mu,timeInt,M,nT),x0,optionsFS);

% Pass the initial guess to fmincon
optionsFM = optimoptions('fmincon','Algorithm','interior-point','Display','iter','TolX',1e-9,'TolCon',1e-4,'TolFun',1e-8,'MaxFunEvals',100000,'MaxIter',100);
[xSol,fval,exitFlag] = fmincon(@(y) ShootingFunctionFMINCONObj(y,M,timeInt,nT,mu),x0,[],[],[],[],[],[],@(y) ShootingFunctionFMINCONConst(y,inCond,fCond,mu,timeInt,M,nT),optionsFM);
