...
int SphericalShapingFunction(
        ...
        double a2, const double h, const double  Vinf, const double  alpha, const double beta)

{
    ...
    double M_dep = DepBody.M0 + DepBody.n*(t_dep-DepBody.t0);
...
//    double Theta_dep = tudat::basic_astrodynamics::orbital_element_conversions::convertEccentricAnomalyToTrueAnomaly(E_dep,DepBody.e);
    ...
    // Check if an hyperbolic excess velocity is available and if so, add it
    Eigen::Vector3d VinfCart;
    if (Vinf == 0)
        VinfCart << 0,0,0;
    else
    ...
    dep_vS = rotMat*(dep_vC + VinfCart);


    // Find position and velocity of Arrival Body
    double M_arr = ArrBody.M0 + ArrBody.n*(t_dep+TOF- ArrBody.t0);
...
//    double Theta_arr = tudat::basic_astrodynamics::orbital_element_conversions::convertEccentricAnomalyToTrueAnomaly(E_arr,ArrBody.e);
    ...
    //Set the parameters for the line search algorithm (to modify the step of the Newton method)
    double step = 2;
    double fixVal = 0.1;
    int ind = 0;
    ...
    Eigen::VectorXd b(10);
    Eigen::VectorXd TOFcomp(maxIter);
    double DeltaTOF = 0.0;
    double Ta2p = 0.0;
    double Ta2m = 0.0;
    double sum = 0.0;
    ...
    while ( (err > toll) && (cont < maxIter) )
    {
                ...
                if (isDpos==1)
        {
            ...
            TOFcomp(cont+1) = (trueThetaStep/6.0)*sum;
        ...
        if ((err > toll)  && (isDpos==1))
        {
                             ...
                             if ((TOFcomp(cont) != 0) && (TOFcomp(cont+1) != 0))
                             {
                                 if (TOFcomp(cont) > TOF)
                                 {
                                     if  ( TOFcomp(cont+1) > TOFcomp(cont) )
                                     {
                                         if (dDeltaTOF > 0)
                                         {
                                             step = -abs(fixVal/(DeltaTOF/dDeltaTOF))*(rand() % 100 + 1)/100;
                                             ind = 1;
                                         }
                                         else
                                         {  step = abs(fixVal/(DeltaTOF/dDeltaTOF))*(rand() % 100 + 1)/100;  }
                                     }
                                     else if ( abs(TOFcomp(cont+1)-TOFcomp(cont))/TOFcomp(cont) < 0.01 )
                                     { if (ind==0)
                                             step = abs(fixVal/(DeltaTOF/dDeltaTOF))*(rand() % 100 + 1)/100;
                                         else
                                             step = -abs(fixVal/(DeltaTOF/dDeltaTOF))*(rand() % 100 + 1)/100;
            ...
            else
            {
                ...
                {
                    isDpos = 0;
                    a2 = (a20-h) + step*2*h*(rand() % 100 + 1)/100;
                }
            }
...
// Calculate results if loop has converged, otherwise return 0
