function [Delta_V, T_Analytic, Kep_Analytic] = Burt_a0_e(initial_orbit, final_orbit, time_int, engine, m0, element_target, constants)
...
theta_E90_initial = atan2(sqrt(1-initial_orbit.e^2), - initial_orbit.e);
...
flag_ecl = 0;
steps = 3000;
options_ODE = odeset('AbsTol',1e-12,'RelTol',1e-12,'events',@(t,x)events_stop_ode_eq(t,x,element_target,final_orbit.e));
...
control.k_a    = 0;
control.k_p    = 0;
control.alfa_a = 3/2*pi*ones(n,1);  
control.alfa_p = pi/2*ones(n,1);
...
% [T_Analytic,Equin_Analytic] = ode113(@(tt,xx) integrand_mod_Burt_a0_e(tt, xx, ts, ...
