...
% t_dep = [8800:50:8900]; % Could be vectorial for grid search
t_dep = [9615:15:9700];
% t_dep = 8850;
l_dep = length(t_dep);
...
fmt = '%d  %d  %d  %.4f  %.4f  %.6f  %.6f  %.6f  %.6f  %.6f  %.6f \n' ;
name = 'Results/results1.txt' ; 
fileId = fopen(name,'w');
...
%             t_out = tic ;
%             fitnessfcn([0.00106485109711665 0.00608760068162705 0.00422578316805302 0.000153942747573291 2.30632306520225 0.767944870877505])
%             t_out = toc(t_out)
%             keyboard
%             
