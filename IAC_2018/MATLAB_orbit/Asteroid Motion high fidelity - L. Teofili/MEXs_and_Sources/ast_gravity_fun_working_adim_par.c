...
void ast_gravity_fun(double rho, double *vert, double *faces, double *rf_, int nfaces,double r[3],double out[3]){
 ...
 //   const double  G0= 6.67259*pow(10.,-11.);
    ...
    #pragma omp parallel for private(i,ii1,ii2,ii3,nc,r1,r2,r3,r1_,r2_,r3_,rf,aa,bb,cc,nf,adot,bdot,cdot,ddot,ri_v,riv,r1_norm,r2_norm,r3_norm,ss,vv,wf,Ff,j,ri,ri_,rj,rj_,ri_norm,rj_norm,dlij,lij,nijf,Eef,Le,re,) reduction(+:gigi1,gigi2,gigi3)
 ...
 for (i=1; i<=nfaces; i++){
	...
	Eef[8] = nijf[2]*nf[2];     
        ...
        gigi1 +=  - Le*(Eef[0]*re[0] + Eef[1]*re[1] + Eef[2]*re[2]);
        gigi2 +=  - Le*(Eef[3]*re[0] + Eef[4]*re[1] + Eef[5]*re[2]);
        gigi3 +=  - Le*(Eef[6]*re[0] + Eef[7]*re[1] + Eef[8]*re[2]);
