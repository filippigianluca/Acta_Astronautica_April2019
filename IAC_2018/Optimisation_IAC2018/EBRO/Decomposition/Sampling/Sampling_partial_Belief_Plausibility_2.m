function [Sample] = Sampling_partial_Belief_Plausibility_2(Partial_curve, decomposition, problem, minmax, minmin, FE_minmax, FE_minmin)
...
for num_coupled_vector = 1:num_coupled_vector_tot
    ...
    num_vector =  problem.num_functions + num_coupled_vector;
    coupled_position = var2opt(num_vector,problem); %in.order_dim_u(sum(in.dim_u_i(1:in.num_functions)) + 1 : end);
    
    
    if problem.dim_u_i(num_vector)>0
        ...
        if num_samples_tot == 1 
            ...
            if problem.flag_output.Belief
                ...
                if ~all(problem.FE_minmax_uc(:,num_coupled_vector))
                    for kk = 1:length(problem.N_ind_components_u{num_vector})       %        size(problem.FE_minmax_uc,1)
                        
                        kkk = problem.N_ind_components_u{num_vector}(kk);
...
%                 coupled_position = in.order_dim_u(sum(in.dim_u_i(1:in.num_functions)) + 1 : end);
        ...
        elseif num_samples_tot == 1 && ~all(problem_decomposition.FE_minmax_uc(:,num_coupled_vector))
