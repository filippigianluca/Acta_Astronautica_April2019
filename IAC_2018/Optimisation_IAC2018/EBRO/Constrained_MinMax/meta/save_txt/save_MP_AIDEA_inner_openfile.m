function algo_inner = save_MP_AIDEA_inner_openfile(problem_minmax, N_u_mpaidea, algo_inner)




    % SAVE POPULATION MP_AIDEA  inner loop
    
    algo_inner.par.save_pop_DE = algo_inner.save_pop_DE;
    % If yes, choose a name for the file:
    name_save_pop_DE =['population_DE_algo_inner' num2str(N_u_mpaidea) '.txt'];
    
    
    algo_inner.par.save_local_search = algo_inner.save_local_search;
    % If yes, choose name for file:
    name_save_local_search = ['minima_fmincon_algo_inner' num2str(N_u_mpaidea) '.txt'];
    
    
    % -------------------------------------------------------------------------
    % Save populations at local restart (each one saved on a different file)?
    % -------------------------------------------------------------------------
    algo_inner.par.save_pop_LR = algo_inner.save_pop_LR;
    % If yes, choose prefix of name for files:
    name_save_pop_LR = ['pop_LR_algo_inner' num2str(N_u_mpaidea)];
    
    % -------------------------------------------------------------------------
    % Save populations at global restart (each one saved on a different file)?
    % -------------------------------------------------------------------------
    algo_inner.par.save_pop_GR = algo_inner.save_pop_GR;
    % If yes, choose prefix of name for files:
    name_save_pop_GR = ['pop_GR_algo_inner' num2str(N_u_mpaidea)];
    
    
    
    % File to save population of DE
    if algo_inner.par.save_pop_DE
        algo_inner.par.str = '%18.16f';
        for iii = 1 : problem_minmax.dim_u
            algo_inner.par.str = [algo_inner.par.str,' ', '%18.16f'];
        end
        algo_inner.par.str = [algo_inner.par.str, '\n'];
        for s = 1 : algo_inner.par.n_populations
%             algo_inner.par.fileID(s) = fopen(name_save_pop_DE,'w');
