function [M,P,info] = space_power_5subsystems(x,ep)
...
%
% ADDED BY Gianluca
%         ep_power(13) =    Delta rho_sa [% kg/m^2] density solar array, [10 15][15 20]
%         ep_power(14) =    Delta D_cell [%] degradation solar array,    [10 15][15 20]
...
% ADDED BY GIANLUCA
Delta.rho = ep(13);
Delta.degradation = ep(14);
%---------------------

[M_array, P_array, array, eta] = solar_array(Preq, Pecl, tsun, tecl, range, theta, life, V_sa, cell_type, configuration, eta, MeV, Tmargin, Delta);
...
function [M_array, P_array, array, eta] = solar_array(Preq, Pecl, tsun, tecl, range, theta, life, V_bus, cell_type, configuration, eta, MeV, Tmargin, Delta)
...
if make_continuous_d
    ...
    spec_mass =  [32e-2 116e-2 86e-2];
    efficiency = [0.169*interp1(fluences,[1,0.80,0.74,0.64],MeV,'linear','extrap')  ...
                        interp1(fluences,[0.28,0.266,0.255,0.245],MeV,'linear','extrap')  ...
                        interp1(fluences,[0.293,0.276,0.267,0.254],MeV,'linear','extrap')];
    degradation= [0.0375  0.0275  0.0275];
    
    cell.spec_mass   = interp1(x_int,spec_mass,cell_type,'linear','extrap');
    cell.efficiency  = interp1(x_int,efficiency,cell_type,'linear','extrap');
    cell.degradation = interp1(x_int,degradation,cell_type,'linear','extrap');
...
%---------------------
% GIANLUCA
cell.degradation = cell.degradation*(1+Delta.degradation/100);
cell.spec_mass   = cell.spec_mass*(1+Delta.rho/100);
%---------------------
...
if make_continuous_d
    ...
    spec_energy = [115  118  133  139  155  126  165];
    
    
    cell.spec_energy   = interp1(x_int,spec_energy,selected,'linear','extrap');
...
M_batteries = Ecap/cell.spec_energy;
