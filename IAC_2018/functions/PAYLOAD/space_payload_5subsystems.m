function [M,P_imaging,P_idle,info] = space_payload_5subsystems(x,ep, par)
%
...
cost_r1 = 0.5;  % rate functionality: state2/state1

mu = astro_constants(13); % G*M  (km3 s-2)
R  = astro_constants(23); % earth radius (km)
...
tt_day    = x(1);         % mission time in days
tt_sec    = x(1)*86400;   % mission time in seconds
...
h            = ep(1);      % altitude   (km)
epsilon_min  = ep(2);      % elevation angle (deg)
...
T_orbit = 2*pi*((R+h)^3/mu)^0.5;   % seconds
...
info.t_day   = T_orbit - ECLIPSE; % seconds
info.t_night = ECLIPSE;           % seconds
%--------------------------------------------------------------------------
...
if make_continuous_d
    ...
    M               = interp1([0 1/3 2/3 1],[1.1 1.1 0.256 1.1],x(3),'linear','extrap');
    P_imaging       = interp1([0 1/3 2/3 1],[4 4 2.5 4],x(3),'linear','extrap');
    P_idle          = interp1([0 1/3 2/3 1],[0 0 1.75 9.75],x(3),'linear','extrap');
    info.image_size = interp1([0 1/3 2/3 1],[1280*1024 640*480 2592*1944 1280*1024],x(3),'linear','extrap');
    info.frame_rate = interp1([0 1/3 2/3 1],[6.6 26.6 26.6 26.6],x(3),'linear','extrap');
    
%     M=M*(1+2*bit_depth/100);
else
...
% use the frame rate from the catalog
%
...
TIME_vector = 0:tt_day/10:tt_day;

% dt = 0*ones(1,length(TIME_vector));
% N_images_vector = floor(info.frame_rate*T_orbit*(1-dt/T_orbit));
N_images_vector = info.frame_rate*T_orbit*ones(1,length(TIME_vector));


% N_images_vector = N_images_vector.*(1-wblcdf(TIME_vector, scale, beta));
...
Data_Volume_max = info.image_size*bit_depth/8/2^30*max(N_images_vector);
Data_Volume_tot = info.image_size*bit_depth/8/2^30.*(N_images_vector);


R2 = interp1(TIME_vector, Data_Volume_tot, tt_day,'linear','extrap');
R1 = R2*cost_r1;

R1_function = @(x)(R1fun(TIME_vector,Data_Volume_tot,x));
R2_function = @(x)(R2fun(TIME_vector,Data_Volume_tot,x));
...
info.Data_Volume_max                 = Data_Volume_max;                        % (Giga Bytes)
...
info.R1_vector = Data_Volume_tot/2;
info.R2_vector = Data_Volume_tot;
