function [M,P,info] = space_ttc_5subsystems(x,ep)
...
% Link budget
EbNo = BER2EbNo(BER, modulation, beta, x(2));
PtdB = EbNo - Gt - Lt - Ls - Ltotal - GoverT + 10*log10(R) - 228.6 + link_margin;
Pt = 10^(PtdB/10);

% Transponder
[M_transponder, P_transponder, transponder] = transponder_design(Pt, amplifier, x(9));
...
function [M_transponder, P_transponder, transponder] = transponder_design(Pt, amplifier, x_amp)
...
%     x_int = [0 1];
%        
%     % mass and power of SSPA
%     x1_0 = [1.4 2   4   6   8   10  20  30  40  50  60  70  80];
%     y1_0 = [1.2 1.2 1.2 1.2 1.2 1.2 1.3 1.5 1.7 1.9 2.2 2.8 3.5];
%     M_0 = interp1(log10(x1_0),log10(y1_0),log10(Pt),'linear','extrap');
%     M_0 = 10^M_0;
%     x2_0 = [0.1 0.5  2 4   6  8 10 20 30 40 50   60  70];
%     y2_0 = [9   12  20 28 30 32 35 50 70 90 130 180 200];
%     P_0 = interp1(log10(x2_0),log10(y2_0),log10(Pt),'linear','extrap');
%     P_0 = 10^P_0;
% 
%     % mass and power of TWTA
%     x1_1 = [0.5 0.7 0.9 2   4   6 8   10  20 30 40   50  60  70 80];
%     y1_1 = [1.8 1.9 2   2.3 2.8 3 3.3 3.6 4  5  5.8  6.2 6.5  7  8];
%     M_1 = interp1(log10(x1_1),log10(y1_1),log10(Pt),'linear','extrap');
%     M_1 = 10^M_1;
%     x2_1 = [0.1 0.5  1  2   4   6   8  10  20 30 40   50   60  70  80  90];
%     y2_1 = [8   9   10 14  16  18  21  24  32 45 60   80  105 130 155 180];
%     P_1 = interp1(log10(x2_1),log10(y2_1),log10(Pt),'linear','extrap');
%     P_1 = 10^P_1;    
%     
%     
%     M_int = [M_0 M_1];
%     P_int = [P_0 P_1];
%     
%     M_transponder  = interp1(x_int,M_int,x_amp,'linear','extrap');
%     P_transponder  = interp1(x_int,P_int,x_amp,'linear','extrap');
%     
%     M_transponder = M_transponder*2;
% else
...
function EbNo = BER2EbNo(BER, modulation, beta, x_mod)

make_continuous_d = 1;

if make_continuous_d
    
    x_int = [0 1/6 2/6 3/6 4/6 5/6 1];
    y_int = [(erfcinv(2*BER))^2  1/0.6*(erfcinv(2*BER))^2  sqrt(2)*(erfcinv(2*BER)) -2*log(2*BER)   -log(2*BER)  (erfcinv(2*BER))^2  1/(0.5*(1-sin(2*pi*beta)/(2*pi*beta)))*(erfcinv(2*BER))];
    EbNo  = interp1(x_int,y_int,x_mod,'linear','extrap');
