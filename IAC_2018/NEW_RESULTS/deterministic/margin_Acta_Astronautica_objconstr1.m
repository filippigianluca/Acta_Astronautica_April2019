function output = margin_Acta_Astronautica_objconstr1(d, u, par) 
...
% epistemic
ep_payload(1) = u(16);      % h, altitude, epistemic (km)  [600 800][800 1000]
ep_payload(2) = u(17);      % elevation angle (deg)        [0 10][10 20]
ep_payload(3) = u(18);      % delta inclination orbit (%)  [0 10][10 20]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ep_obdh(5) = u(19); % \% mass   [0 10][10 20]
ep_obdh(6) = u(20); % \% power  [0 10][10 20]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ep_power(10) = u(13);  %       * ep(10) = uncertainty on power requirements, %                   (ALICINO uncertain, [0 10][10 20])

%-- add to the function "space_power_Alicino_paper":
ep_power(13) = u(14);  %        uncertain         Delta rho_sa [% kg/m^2] density solar array, [0 15][15 30]
ep_power(14) = u(15);  %        uncertain         Delta D_cell [%] degradation solar array,    [0 50][50 100]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ep_obdh(4) = info_payload.Data_Volume_max;                   % data volume total            (Giga Bytes)
%--------------------------------------------------------------------------
...
ep_ttc(8) = info_payload.Tac_tot*60*par.fix.time;                %       * ep(8) = ground station access time, min         
...
ep_power(1) = 16 + (P_aocs + P_ttc + P_payload_imaging + P_obdh)*1.2;    %       * ep(1) = required daylight power, W                      (ALICINO transfer function)
ep_power(2) = 16 + (P_aocs + P_ttc + P_payload_idle    + P_obdh)*1.2;        %       * ep(2) = required eclipse power, W                       (ALICINO transfer function)
...
%% output
%--------------------------------------------------------------------------
output(1) = (M_payload + M_obdh + M_aocs + M_ttc + M_power)*1.2;
%--------------------------------------------------------------------------
...
%% output
%--------------------------------------------------------------------------
output(2) = (M_payload + M_obdh + M_aocs + M_ttc + M_power)*1.2;
%--------------------------------------------------------------------------
